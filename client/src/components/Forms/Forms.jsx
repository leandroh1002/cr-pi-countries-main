...
function Forms() {
  ...
  useEffect(() => {
    const fetchCountries = async () => {
      try {
        const response = await axios.get('https://countriesback-l6bu.onrender.com/api/countries');
  ...
  const handleSubmit = async (event) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        const response = await axios.post('https://countriesback-l6bu.onrender.com/api/activities', userData);
